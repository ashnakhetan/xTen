...
export class ChatBotPlugin extends OpenAIPlugin {
    constructor(apiKey, conversationContext, history, maxHistory, maxMessageLength, personality) {
        super(apiKey);  // Call the constructor of the parent class (OpenAIPlugin)
    ...
    async askQuestion(userQuestion) {
        try {
            ...
            const response = await this.chat(this.history); // Using the chat method from the inherited OpenAIPlugin
    ...
    async askSystemQuestion(userQuestion) {
        try {
            let userRole = "system";
            this.addMessageToHistory(userRole, userQuestion);
            const response = await this.chat(this.history); // Using the chat method from the inherited OpenAIPlugin
    ...
    async initializeSession() {
        ...
        try {
            ...
            const sessionMessage = await this.chat(this.history);
